...
const MISSOES_DISPONIVEIS = [
  { id: 1, icon: 'walk', title: 'Caminhada Matinal', desc: 'Caminhe 5.000 passos hoje', detalhes: 'Caminhar regularmente melhora a saúde cardiovascular e ajuda a manter o peso ideal.', pts: 50, progress: 0, done: false, color: 'var(--color-green)', grad: 'linear-gradient(90deg,var(--color-green2),var(--color-green))', offset: 144.51 },
  { id: 2, icon: 'water', title: 'Hidratação', desc: 'Beba 2 litros de água', detalhes: 'Manter-se hidratado é essencial para o bom funcionamento do organismo e melhora a disposição.', pts: 30, progress: 0, done: false, color: 'var(--color-teal)', grad: 'linear-gradient(90deg,#009e9a,var(--color-teal))', offset: 144.51 },
  { id: 3, icon: 'meditation', title: 'Alongamento', desc: 'Faça 10 minutos de alongamento', detalhes: 'Alongar-se diariamente previne lesões, melhora a postura e reduz tensões musculares.', pts: 40, progress: 0, done: false, color: 'var(--color-blue)', grad: 'linear-gradient(90deg,#2e6fff,var(--color-blue))', offset: 144.51 },
  { id: 7, icon: 'apple', title: 'Alimentação Saudável', desc: 'Registre 3 refeições saudáveis', detalhes: 'Uma alimentação balanceada fornece energia e nutrientes essenciais para o dia.', pts: 60, progress: 0, done: false, color: 'var(--color-amber)', grad: 'linear-gradient(90deg,#c07800,var(--color-amber))', offset: 144.51 },
  { id: 8, icon: 'sleep', title: 'Sono de Qualidade', desc: 'Durma 7-8 horas', detalhes: 'Um sono reparador é fundamental para a recuperação física e mental.', pts: 50, progress: 0, done: false, color: '#7c3aed', grad: 'linear-gradient(90deg,#7c3aed,#a78bfa)', offset: 144.51 },
  { id: 9, icon: 'book', title: 'Leitura Diária', desc: 'Leia por 20 minutos', detalhes: 'A leitura estimula o cérebro, reduz o estresse e melhora a concentração.', pts: 35, progress: 0, done: false, color: '#6366f1', grad: 'linear-gradient(90deg,#4338ca,#6366f1)', offset: 144.51 },
  { id: 10, icon: 'dumbbell', title: 'Treino de Força', desc: 'Faça 30 minutos de musculação', detalhes: 'Exercícios de força aumentam a massa muscular e aceleram o metabolismo.', pts: 80, progress: 0, done: false, color: '#ea580c', grad: 'linear-gradient(90deg,#ea580c,#fb923c)', offset: 144.51 },
]
  ...
  return (
    <div className="flex h-screen">
      ...
      <div className="flex flex-1 flex-col overflow-hidden">
        <Topbar title="Missões" subtitle="Acompanhe seu progresso diário e semanal" />
            ...
            <div className="grid grid-cols-[repeat(auto-fill,minmax(380px,1fr))] gap-3.5">
              {missoesAtivas.length === 0 ? (
                <div className="flex min-w-full flex-col items-center justify-center rounded-[18px] border-[1.5px] border-dashed border-border2 bg-surface px-5 py-[60px] text-center">
                  <div className="mb-3 text-muted opacity-40"><Icon name="target" className="mx-auto h-12 w-12" /></div>
...
function MissionCardExpanded({ mission, index, expanded, onToggle, onAdd, onRemove, isAvailable }) {
  const { icon, title, desc, detalhes, pts, progress, done, color, grad, offset } = mission
    ...
    >
      <div className="relative flex items-center gap-3.5 px-5 py-[18px]">
        <div className="relative flex-shrink-0">
          ...
          <div className="absolute inset-0 flex items-center justify-center" style={{ color }}><Icon name={icon} className="h-[22px] w-[22px]" /></div>
        </div>

        <div className="min-w-0 flex-1 cursor-pointer" onClick={onToggle}>
          <div className="mb-1 flex items-center gap-2">
            <div className="min-w-0 truncate font-display text-[15px] font-semibold" style={{ color }}>{title}</div>
